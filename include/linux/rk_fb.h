...
struct rk_lcdc_drv_ops {
	...
	int (*set_bg_color) (struct rk_lcdc_driver *dev_drv, u8 R, u8 G, u8 B);
	int (*win0_enable) (struct rk_lcdc_driver *dev_drv, int enable, u32 addr);
	int (*win1_enable) (struct rk_lcdc_driver *dev_drv, int enable, u32 addr);
	int (*hwc_enable) (struct rk_lcdc_driver *dev_drv, int enable, u32 addr);

	int (*win0_act_info)(struct rk_lcdc_driver *dev_drv, u16 xact, u16 yact);
	int (*win0_dsp_info)(struct rk_lcdc_driver *dev_drv, u16 xsize, u16 ysize);
	int (*win0_dsp_st)(struct rk_lcdc_driver *dev_drv, u16 x, u16 y);
	int (*win0_vir)(struct rk_lcdc_driver *dev_drv, u16 x, u16 y);

	int (*win1_act_info)(struct rk_lcdc_driver *dev_drv, u16 xact, u16 yact);
	int (*win1_dsp_info)(struct rk_lcdc_driver *dev_drv, u16 xsize, u16 ysize);
	int (*win1_dsp_st)(struct rk_lcdc_driver *dev_drv, u16 x, u16 y);
	int (*win1_vir)(struct rk_lcdc_driver *dev_drv, u16 x, u16 y);
	int (*hwc_dsp_st)(struct rk_lcdc_driver *dev_drv, u16 x, u16 y);
	int (*hwc_size)(struct rk_lcdc_driver *dev_drv, u8 size);
	phys_addr_t (*iova_to_phy)(struct rk_lcdc_driver *dev_drv, void* addr);
	int (*hwc_alpah_en)(struct rk_lcdc_driver *dev_drv, u8 enable);
...
/* ビデオ初期化 */
typedef struct video_output_ini {
	int			bitblt_max;		/* BitBlt最大数*/
	int			alloc_max;		/* alloc_buffer最大数*/
	uint16_t	video_width;	/* ビデオ横幅 */
	uint16_t	video_height;	/* ビデオ縦幅 */
	uint16_t	win0_width;		/* Win0横幅 */
	uint16_t	win0_height;	/* Win0縦幅 */
	uint16_t	win1_width;		/* Win1横幅 */
	uint16_t	win1_height;	/* Win1縦幅 */
	enum hwc_size hwc_size;		/* Hwc幅 */
	int			hwc_lut_num;	/* Hwc用LUTの数 */
}__attribute__((packed)) VIDEO_OUTPUT_INI;

typedef struct bitblt_list {
	uint32_t	dst_addr;
	...
	uint16_t	width;
	uint16_t	height;
	enum pixel_type mode;
}__attribute__((packed)) BITBLT_LIST;

struct list_data {/* list data */
	BITBLT_LIST req;
	struct list_head list;
};
...
/* 初期化 */
#define RK_FB_VIDEO_INIT           0x4000
#define RK_FB_VIDEO_EXIT           0x4001
/* bitblt */
#define RK_FB_SET_BITBLT           0x4010
#define RK_FB_BITBLT_START         0x4011
#define RK_FB_BITBLT_WAIT          0x4012
/* バッファ取得 */
#define RK_FB_VIDEO_BUF_GET        0x4020
#define RK_FB_VIDEO_BUF_PUT        0x4021
#define RK_FB_FRAME_BUFFER_GET     0x4022
#define RK_FB_FRAME_BUFFER_PUT     0x4023
#define RK_FB_BACK_BUFFER_NUM_GET  0x4025
/* フリップ実行 */
#define RK_FB_FLIP_START           0x4030
#define RK_FB_FLIP_WAIT            0x4031
/* layer設定 */
#define RK_FB_SET_BLACK_DISP       0x4040
#define RK_FB_SET_BACKGROUND_COROL 0x4041
#define RK_FB_SET_WIN0_ENABLE      0x4042
#define RK_FB_SET_WIN0_CONDITION   0x4043
#define RK_FB_SET_WIN1_ENABLE      0x4044
#define RK_FB_SET_WIN1_CONDITION   0x4045
#define RK_FB_SET_HWC_ENABLE       0x4046
#define RK_FB_SET_HWC_CONDITION    0x4047
#define RK_FB_SET_HWC_LUT_NUM      0x4048
#define RK_FB_SET_HWC_LUT_BUF_GET  0x4049
#define RK_FB_SET_HWC_LUT_BUF_PUT  0x404a
/* sync */
#define RK_BUFFER_SYNC             0x4050
