...
static int rk312x_lcdc_bg_color(struct rk_lcdc_driver *dev_drv, u8 R, u8 G, u8 B)
{
	u32 mask, val;
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	mask = m_BG_COLOR;
	val = v_BG_B(B)|v_BG_G(G)|v_BG_R(R);
	spin_lock(&lcdc_dev->reg_lock);
	lcdc_msk_reg(lcdc_dev, DSP_CTRL1, mask, val);
	lcdc_cfg_done(lcdc_dev);
...
static int rk312x_lcdc_win0_enable(struct rk_lcdc_driver *dev_drv, int enable, u32 addr)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	...
	lcdc_msk_reg(lcdc_dev, SYS_CTRL, m_WIN0_EN, v_WIN0_EN(enable));
	lcdc_msk_reg(lcdc_dev, SYS_CTRL, m_WIN0_FORMAT, v_WIN0_FORMAT(VOP_FORMAT_RGB565));
	if(enable){
		lcdc_writel(lcdc_dev, WIN0_YRGB_MST, addr);
	}
	else{
...
static int rk312x_lcdc_win1_enable(struct rk_lcdc_driver *dev_drv, int enable, u32 addr)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);

	spin_lock(&lcdc_dev->reg_lock);
	lcdc_msk_reg(lcdc_dev, SYS_CTRL, m_WIN1_EN, v_WIN1_EN(enable));
	lcdc_msk_reg(lcdc_dev, SYS_CTRL, m_WIN1_FORMAT, v_WIN1_FORMAT(VOP_FORMAT_RGB565));
	if(enable){
		if (lcdc_dev->soc_type == VOP_RK3036)
			lcdc_writel(lcdc_dev, WIN1_MST, addr);
		else
...
static int rk312x_lcdc_hwc_enable(struct rk_lcdc_driver *dev_drv, int enable, u32 addr)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	...
	lcdc_msk_reg(lcdc_dev, SYS_CTRL, m_HWC_EN | m_HWC_LODAD_EN, 
				 v_HWC_EN(enable) | v_HWC_LODAD_EN(enable));

	if(enable){
		lcdc_writel(lcdc_dev, HWC_MST, addr);
	}
	else{
		lcdc_writel(lcdc_dev, HWC_MST, 0);
	}
	lcdc_cfg_done(lcdc_dev);
	spin_unlock(&lcdc_dev->reg_lock);
...
static int rk312x_lcdc_win0_act_info(struct rk_lcdc_driver *dev_drv, u16 xact, u16 yact)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	spin_lock(&lcdc_dev->reg_lock);
...
static int rk312x_lcdc_win0_dsp_info(struct rk_lcdc_driver *dev_drv, u16 xsize, u16 ysize)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	spin_lock(&lcdc_dev->reg_lock);
	lcdc_writel(lcdc_dev, WIN0_DSP_INFO, v_DSP_WIDTH(xsize) |
		    v_DSP_HEIGHT(ysize));
	lcdc_cfg_done(lcdc_dev);
	spin_unlock(&lcdc_dev->reg_lock);
...
static int rk312x_lcdc_win0_dsp_st(struct rk_lcdc_driver *dev_drv, u16 x, u16 y)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	spin_lock(&lcdc_dev->reg_lock);
	lcdc_writel(lcdc_dev, WIN0_DSP_ST, v_DSP_STX(x) | v_DSP_STY(y));
	lcdc_cfg_done(lcdc_dev);
	spin_unlock(&lcdc_dev->reg_lock);
...
static int rk312x_lcdc_win0_vir(struct rk_lcdc_driver *dev_drv, u16 x, u16 y)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	spin_lock(&lcdc_dev->reg_lock);
...
static int rk312x_lcdc_win1_dsp_st(struct rk_lcdc_driver *dev_drv, u16 x, u16 y)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	spin_lock(&lcdc_dev->reg_lock);
	if (lcdc_dev->soc_type == VOP_RK3036)
		lcdc_writel(lcdc_dev, WIN1_DSP_ST, v_DSP_STX(x) | v_DSP_STY(y));
	else if (lcdc_dev->soc_type == VOP_RK312X)
		lcdc_writel(lcdc_dev, WIN1_DSP_ST_RK312X, v_DSP_STX(x) | v_DSP_STY(y));
...
static int rk312x_lcdc_win1_vir(struct rk_lcdc_driver *dev_drv, u16 x, u16 y)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	spin_lock(&lcdc_dev->reg_lock);
...
static int rk312x_lcdc_win1_act_info(struct rk_lcdc_driver *dev_drv, u16 xact, u16 yact)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	spin_lock(&lcdc_dev->reg_lock);

	lcdc_writel(lcdc_dev, WIN1_ACT_INFO, v_ACT_WIDTH(xact) |
		    v_ACT_HEIGHT(yact));
	lcdc_cfg_done(lcdc_dev);
	spin_unlock(&lcdc_dev->reg_lock);
...
static int rk312x_lcdc_win1_dsp_info(struct rk_lcdc_driver *dev_drv, u16 xsize, u16 ysize)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	spin_lock(&lcdc_dev->reg_lock);
	if (lcdc_dev->soc_type == VOP_RK3036)
		lcdc_writel(lcdc_dev, WIN1_DSP_INFO, v_DSP_WIDTH(xsize) |
					v_DSP_HEIGHT(ysize));
	else if (lcdc_dev->soc_type == VOP_RK312X)
		lcdc_writel(lcdc_dev, WIN1_DSP_INFO_RK312X, v_DSP_WIDTH(xsize) |
					v_DSP_HEIGHT(ysize));
...
static int rk312x_lcdc_hwc_dsp_st(struct rk_lcdc_driver *dev_drv, u16 x, u16 y)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	spin_lock(&lcdc_dev->reg_lock);

	lcdc_writel(lcdc_dev, HWC_DSP_ST, v_DSP_STX(x) | v_DSP_STY(y));
...
static int rk312x_lcdc_hwc_size(struct rk_lcdc_driver *dev_drv, u8 size)
{
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
	spin_lock(&lcdc_dev->reg_lock);

	lcdc_msk_reg(lcdc_dev, SYS_CTRL, m_HWC_SIZE, v_HWC_SIZE(size));
...
static phys_addr_t rk312x_iova_to_phy(struct rk_lcdc_driver *dev_drv, void* addr)
{

	if (dev_drv->iommu_enabled) {
		if (dev_drv->mmu_dev) {
			struct device *dev = dev_drv->dev;
			phys_addr_t phy;
			extern phys_addr_t rockchip_iovmm_iova_to_phy(struct device *dev, void* addr);

			phy = rockchip_iovmm_iova_to_phy(dev, addr);
			return phy;
		}
	}
	return 0;
...
static int rk312x_hwc_alpah_en(struct rk_lcdc_driver *dev_drv, u8 enable)
{
	u32 mask, val;
	struct lcdc_device *lcdc_dev =
			container_of(dev_drv, struct lcdc_device, driver);
...
#if 1 /* 2015/06 LCD_CHANGE */
	.set_bg_color = rk312x_lcdc_bg_color,
	.win0_enable = rk312x_lcdc_win0_enable,
	.win1_enable = rk312x_lcdc_win1_enable,
	.hwc_enable = rk312x_lcdc_hwc_enable,
	.win0_act_info = rk312x_lcdc_win0_act_info,
	.win0_dsp_info = rk312x_lcdc_win0_dsp_info, 
	.win0_dsp_st = rk312x_lcdc_win0_dsp_st,
	.win0_vir = rk312x_lcdc_win0_vir,
	.win1_act_info = rk312x_lcdc_win1_act_info,
	.win1_dsp_info = rk312x_lcdc_win1_dsp_info, 
	.win1_dsp_st = rk312x_lcdc_win1_dsp_st,
	.win1_vir = rk312x_lcdc_win1_vir,
	.hwc_dsp_st = rk312x_lcdc_hwc_dsp_st,
	.hwc_size  = rk312x_lcdc_hwc_size,
	.iova_to_phy = rk312x_iova_to_phy,
	.hwc_alpah_en = rk312x_hwc_alpah_en,
#endif /* 2015/06 LCD_CHANGE */
};
#if 0
