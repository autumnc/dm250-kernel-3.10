...
static LIST_HEAD(cmd_reg_memory);
int cmd_reg_memory_init(int num)
{
	struct rga_reg *reg;
	int i;
	for(i = 0; i < num; i++){
		reg = kzalloc(sizeof(struct rga_reg), GFP_KERNEL);
		if(reg == NULL){
			return -ENOMEM;
		}
		list_add_tail(&reg->list, &cmd_reg_memory);
	}
	return 0;
}

int cmd_reg_memory_exit(void)
{
	while(!list_empty(&cmd_reg_memory)){
		struct rga_reg *reg;
		reg = list_entry(cmd_reg_memory.next, struct rga_reg, list);
		list_del(&reg->list);
		kfree(reg);
	}
	return 0;
...
static struct rga_reg * rga_reg_init(rga_session *session, struct rga_req *req)
{
	...
	if(list_empty(&cmd_reg_memory)){
		printk("%s, [%d] cmd reg alloc failed\n", __FUNCTION__, __LINE__);
		return NULL;
	}
	reg = list_entry(cmd_reg_memory.next, struct rga_reg, list);
	list_del(&reg->list);
...
static int rga_drv_suspend(struct platform_device *pdev)
{
	rga_soft_reset();
	return 0;
}
static int rga_drv_resume(struct platform_device *pdev)
{
	rga_soft_reset();
	return 0;
}
...
#if defined(CONFIG_PM)
	.suspend	= rga_drv_suspend,
	.resume		= rga_drv_resume,
#endif
	},
};
