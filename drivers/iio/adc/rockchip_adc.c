...
static int rk_adc_suspend(struct device *dev)
{
	...
	if (pm_device_down) {
		...
		clk_enable(info->clk);
		clk_enable(info->pclk);
		pm_regs[0] = readl_relaxed(info->regs + ADC_CTRL) & ~(1 << 6);
		pm_regs[1] = readl_relaxed(info->regs + ADC_DELAY_PU_SOC);
		if (readl_relaxed(info->regs + ADC_CTRL) & (1 << 6))
			writel_relaxed(pm_regs[0], info->regs + ADC_CTRL);
		clk_disable(info->clk);
		clk_disable(info->pclk);
	}
#endif
