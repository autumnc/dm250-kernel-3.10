...
static int dbg_enable = 0; static int dbg_qmax = -1; static int dbg_design_cap = -1;
...
extern long	rk818_get_lrtc( void );
...
static void rk818_bat_debug_info(struct rk818_battery *di)
{
	...
	DBG("###############################################################\n"
	    ...
	    "min=%d, init=%d, sw=%d, below0=%d, first=%d, changed=%d, dbg_design_cap=%d, design_cap=%d, dbg_qmax=%d, qmax=%d\n"
	    "###############################################################\n",
	    ...
	    di->dbg_cap_low0, di->is_first_on, di->last_dsoc, dbg_design_cap, di->design_cap, dbg_qmax, di->qmax
	   );
}
...
static void rk818_bat_init_info(struct rk818_battery *di)
{
	...
	di->plug_in_base = 0;
	di->plug_out_base = 0; dbg_design_cap = di->design_cap; dbg_qmax = di->qmax;
}
...
#ifdef	CONFIG_PM_WARP
	if( pm_device_down == WARP_STATE_RESUME )
	{
		long	lrtc	= rk818_get_lrtc( );

		if(    ((lrtc & DM250_MASK_TIMECODE_SIGNBIT) != 0)	/* invalid LRTC (returns -1) or occured Unix2038 problem (RTC can count over the boundary) */
			|| ( tv.tv_sec < lrtc )	/* might occured system clock wrap-around to earlier side of the supporting time range */
		  )
		{
			printk( KERN_INFO "%s(): invalid LRTC has been detected, use current time instead.\n", __func__ );
			di->rtc_base.tv_sec	= tv.tv_sec;	/* causes 0-interval */
		}
		else
		{
			di->rtc_base.tv_sec	= lrtc;
		}
	}
#endif	// CONFIG_PM_WARP
	interval_sec = tv.tv_sec - di->rtc_base.tv_sec;
	printk( KERN_INFO "%s(): now=%li(0x%08lx), suspend_time=%li(0x%08lx) => interval_sec=%i(0x%08x), \n"
		, __func__, tv.tv_sec, tv.tv_sec, di->rtc_base.tv_sec, di->rtc_base.tv_sec, interval_sec, interval_sec );
	return	interval_sec;
}
