...
#if defined(CONFIG_FIQ_DEBUGGER) && defined(CONFIG_PM_WARP)
	u32 saved_igroup[DIV_ROUND_UP(1020, 32)];
#endif
...
#if defined(CONFIG_PM_WARP) && defined(CONFIG_FIQ_DEBUGGER)
	if (pm_device_down) {
		for (i = 0; i < DIV_ROUND_UP(gic_irqs, 32); i++)
			gic_data[gic_nr].saved_igroup[i] =
...
static void gic_dist_restore(unsigned int gic_nr)
{
	...
	if (pm_device_down) {
		for (i = 0; i < DIV_ROUND_UP(gic_irqs, 32); i++) {
			writel_relaxed(gic_data[gic_nr].saved_igroup[i],
