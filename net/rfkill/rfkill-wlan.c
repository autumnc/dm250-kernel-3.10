    ...
    if (mrfkill->pdata->mregulator.power_ctrl_by_pmu) {
		...
		if (on){
			...
			if (gpio_is_valid(poweron->io)) {
				gpio_direction_output(poweron->io, 0);
...
static int rfkill_wlan_pm_resume(struct device *dev)
{
	...
	if (pm_device_down) {
		if (gpio_is_valid(mrfkill->pdata->power_n.io)) {
			gpio_direction_output(mrfkill->pdata->power_n.io, !mrfkill->pdata->power_n.enable);
		}

		if (mrfkill->pdata->wifi_power_remain) {
			rockchip_wifi_power(1);
		}
...
static struct dev_pm_ops rfkill_wlan_pm_ops = {
	.thaw		= rfkill_wlan_pm_resume,
	.restore	= rfkill_wlan_pm_resume,
};
#endif
